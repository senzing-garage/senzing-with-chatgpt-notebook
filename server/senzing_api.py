...
EXPORT_FLAGS_MAP = dict(
    MATCHED                = EXPORT_FLAGS_MATCHED,
    POSSIBLE_MATCHES       = EXPORT_FLAGS_POSSIBLE_MATCHES,
    POSSIBLE_RELATIONSHIPS = EXPORT_FLAGS_POSSIBLE_RELSHIPS,
    )
...
class SenzingAPI:
    ...
    def entity_report(self, export_flags, limit=EXPORT_LIMIT):
        # Export Flags
        if not export_flags in EXPORT_FLAGS_MAP:
            raise Exception('Unrecognized `export_flags`!')
        export_flags = EXPORT_FLAGS_MAP[export_flags]
