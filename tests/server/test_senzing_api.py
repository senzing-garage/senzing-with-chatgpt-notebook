...
class TestSenzingAPI:
    ...
    def test_entity_report_matched(self, api):
        results = api.entity_report('MATCHED')
        assert results == EXPORT_MATCHED_LIMIT10

    def test_entity_report_possible_matches(self, api):
        results = api.entity_report('POSSIBLE_MATCHES')
        assert results == EXPORT_POSSIBLE_MATCHES_LIMIT10

    def test_entity_report_possible_relationships(self, api):
        results = api.entity_report('POSSIBLE_RELATIONSHIPS')
    ...
    def test_entity_search(self, api):
        results = api.entity_search(dict(NAME_FULL='Bob Smith'))
